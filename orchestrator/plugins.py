...
def _builtin_plugin(
    ...
    *,
    ...
    return AdapterPlugin(
        ...
        compatibility={"python": ">=3.11,<3.13", "uta": ">=2.0.0"},
...
def get_builtin_adapter_plugins() -> list[AdapterPlugin]:
    return [
        ...
        _builtin_plugin(
            ...
            fallback_note="Mobile plugin executes deterministic skeleton smoke checks in v2.0.",
        ),
        _builtin_plugin(
            "llm_app",
            LlmAppAdapter,
            "llm_app",
            fallback_mode="skeleton_smoke",
            fallback_note="llm_app plugin executes deterministic skeleton evaluation in v2.0.",
        ),
        _builtin_plugin(
            "rag_app",
            RagAppAdapter,
            "rag_app",
            fallback_mode="skeleton_smoke",
            fallback_note="rag_app plugin executes deterministic retrieval smoke evaluation in v2.0.",
        ),
        _builtin_plugin(
            "workflow",
            WorkflowAdapter,
            "workflow",
            fallback_mode="skeleton_smoke",
            fallback_note="workflow plugin executes deterministic orchestration smoke validation in v2.0.",
        ),
        _builtin_plugin(
            "data_pipeline",
            DataPipelineAdapter,
            "data_pipeline",
            fallback_mode="skeleton_smoke",
            fallback_note="data_pipeline plugin executes deterministic schema/integrity smoke validation in v2.0.",
        ),
    ]
